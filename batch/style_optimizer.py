...
PATTERN_COMBOS: dict[str, set[str]] = {
    ...
    "ALL(A+B+C+D+E)": {"A_trend", "B_pullback", "C_breakout", "D_reversal", "E_can_slim"},
    ...
    for style_name, days_list in STYLES.items():
        for holding_days in days_list:
            for combo_name, allowed in PATTERN_COMBOS.items():
                ...
                if verbose:
                    print(f"[{done:2d}/{total}] {style_name:5s} {holding_days:3d}d x {combo_name}",
                          end=" ... ", flush=True)
                row, eq = _run_one(
                    contexts=contexts, trading_days=trading_days,
                    start_date=start_date, end_date=end_date, strategy=strategy,
                    initial_capital=initial_capital, max_positions=max_positions,
                    scenario_id=sid, style=style_name, holding_days=holding_days,
                    pattern_combo=combo_name, allowed_patterns=allowed,
                    trailing_atr_mult=2.0, trend_exit_period=50, use_time_exit=True,
                    nikkei_total=nikkei_total, nikkei_cagr=nikkei_cagr, verbose=verbose,
                )
                if row: summaries.append(row)
                if eq is not None: curves.append(eq)
...
def run_exit_grid(
    ...
    total = (len(EXIT_GRID_COMBOS) * len(EXIT_GRID_TRAILING)
             * len(EXIT_GRID_SMA_PERIOD) * len(EXIT_GRID_TIME_EXIT))
    done = 0
            ...
            for sma_p in EXIT_GRID_SMA_PERIOD:
                for use_te in EXIT_GRID_TIME_EXIT:
                    ...
                    row, eq = _run_one(
                        ...
                        start_date=start_date, end_date=end_date, strategy=strategy,
                        initial_capital=initial_capital, max_positions=max_positions,
                        scenario_id=sid, style="exit_grid",
                        holding_days=EXIT_GRID_HOLDING,
                        pattern_combo=combo_name, allowed_patterns=allowed,
                        trailing_atr_mult=tr_mult, trend_exit_period=sma_p,
...
def _print_summary(results_df: pd.DataFrame, nikkei_total: float) -> None:
    beats_df = results_df[results_df["beats_nikkei"]].sort_values("cagr", ascending=False)
    print(f"\n{'='*70}")
    print(f"[result] beats Nikkei225: {len(beats_df)} / {len(results_df)} scenarios")
    print(f"{'='*70}")
    cols = ["scenario_id", "total_return_pct", "excess_return_pct",
    ...
    for style_name in ["short", "mid", "long"]:
        sub = results_df[results_df["style"] == style_name].sort_values("cagr", ascending=False)
        if sub.empty:
            continue
        best = sub.iloc[0]
    ...
    if not eg.empty:
        ...
        eg_cols = ["scenario_id", "total_return_pct", "excess_return_pct",
                   "cagr", "max_drawdown", "sharpe", "beats_nikkei"]
...
def main() -> None:
    ...
    ap.add_argument("--skip-style-grid", action="store_true",
    ...
    if not args.skip_style_grid:
        ...
        sg_df, sg_eq = run_style_grid(
            ...
            strategy=strategy, nikkei_bench=nikkei_bench,
            initial_capital=args.initial, max_positions=args.max_positions,
        )
        if not sg_df.empty:
    ...
    n_phase2 = (len(EXIT_GRID_COMBOS) * len(EXIT_GRID_TRAILING)
                * len(EXIT_GRID_SMA_PERIOD) * len(EXIT_GRID_TIME_EXIT))
    ...
    equity_all = _add_nikkei_equity(equity_all, bench, start_date, end_date, args.initial)
